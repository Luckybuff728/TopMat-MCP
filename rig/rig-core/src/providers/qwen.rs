...
impl completion::CompletionModel for CompletionModel {
    ...
    > {
        ...
        async move {
            ...
            if response.status().is_success() {
                ...
                // 记录请求 ID
                span.record("gen_ai.response.id", &response.request_id);
                // 记录输出消息
                span.record(
                    "gen_ai.output.messages",
                    serde_json::to_string(&response.output.choices).unwrap(),
                );
                // 记录输入令牌数
    ...
    > {
        ...
        // 构建 HTTP 请求
        let builder = self.client.post("text-generation/generation").json(&request);
        ...
        // 使用追踪工具发送流式请求
        tracing::Instrument::instrument(send_qwen_streaming_request(builder), span).await
    }
}
...
// 发送通义千问流式请求
pub async fn send_qwen_streaming_request(
    // 请求构建器
...
> {
    ...
    // 创建事件源（SSE 客户端）
    // 通义千问使用 X-DashScope-SSE: enable 头来启用流式传输
