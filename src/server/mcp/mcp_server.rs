...
impl ServerHandler for TopMatMcpServer {
    ...
    async fn call_tool(
        ...
        if let Some(ref args) = arguments {
            info!(
                "📥 请求体 (arguments): {}",
                serde_json::to_string_pretty(&serde_json::Value::Object(args.clone()))
                    .unwrap_or_default()
            );
        } else {
            info!("📥 请求体 (arguments): {{}}");
        ...
        if let Some(http_parts) = context.extensions.get::<axum::http::request::Parts>() {
            let headers_str: String = http_parts
                .headers
                .iter()
        ...
        // 提取用户信息和会话上下文
        let (user_id, session_id, transport_type, endpoint) = Self::extract_context_info(&context);
        ...
        if name.starts_with("calphamesh_") {
            ...
            if let Some(http_parts) = context.extensions.get::<axum::http::request::Parts>() {
                ...
                // 尝试从中提取 API key
                if let Some(api_key) =
                    crate::server::auth::extract_api_key_from_headers(&http_parts.headers)
                {
                    info!(
                        "从 HTTP headers 成功提取 API key: {}...",
        ...
        match ToolRegistry::call_tool(&name, args_value.clone()).await {
            Ok(result) => {
                info!("工具 {} 执行成功", name);
                ...
                if let (Some(user_id), Some(transport_type), Some(endpoint)) =
                    ...
                    && let Err(e) = Self::record_tool_call_to_db(
                        user_id,
                        session_id.as_deref(),
                        &name,
                        &args_value,
...
impl TopMatMcpServer {
    ...
    fn extract_context_info(
        ...
        // 尝试从MCP会话上下文中获取信息
        if let Some(mcp_context) = context
            .extensions
            .get::<crate::server::middleware::mcp_storage::McpSessionContext>(
        ) {
        ...
        if user_id.is_none()
            && let Some(auth_user) = context
                .extensions
                .get::<crate::server::middleware::auth::AuthUser>()
        {
