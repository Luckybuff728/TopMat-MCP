...
impl ToolRegistry {
    ...
    //             // 反序列化参数
    //             let args: T::Args =
    //                 serde_json::from_value(args).map_err(|e| format!("参数解析失败: {}", e))?;
    ...
    /// 调用工具（使用静态单例）
    pub async fn call_tool(name: &str, arguments: JsonValue) -> Result<String, String> {
    ...
    pub async fn get_tool_definitions() -> Vec<rmcp::model::Tool> {
        ...
        registry
            ...
            .map(|(name, entry)| rmcp::model::Tool {
                ...
                input_schema: std::sync::Arc::new(entry.input_schema.as_object().unwrap().clone()),
                annotations: None,
                icons: None,
                output_schema: None,
            })
