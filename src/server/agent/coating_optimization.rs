...
pub async fn coating_optimization(
    ...
    handle_chat_request(requirement_agent, request.clone()).await;
    // 【阶段二：性能预测】
    handle_chat_request(prediction_agent, request.clone()).await;
    // 【阶段三：优化建议】
    handle_chat_request(composition_optimizer, request.clone()).await;
    // P2: 结构优化
    println!("\n--- P2: 结构优化 ---\n");
    handle_chat_request(structure_optimizer, request.clone()).await;
    // P3: 工艺优化

    handle_chat_request(process_optimizer, request.clone()).await;
    // 【阶段四：迭代优化】
